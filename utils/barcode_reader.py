...
def search_item_by_barcode(barcode):
    ...
    """
    query = "SELECT id, nome, quantidade FROM ferramentas WHERE codigo_barra = ?"
    resultado = executar_query(query, (barcode,), fetch=True)
    if resultado and resultado[0]:
        item_id, nome, quantidade = resultado[0]
        return {"id": item_id, "nome": nome, "quantidade": quantidade}
    return None
...
def get_item_from_barcode():
    ...
    try:
        ...
        if item:
            print(f"✅ Item encontrado: {item['nome']} (Quantidade disponível: {item['quantidade']})")
            return item
        else:
